...
void stopDeterrent() {
  ...
  ledcWrite(BUZZER_CHANNEL, 0); // 0% duty cycle = silent
}

void playSiren(double freq) {
  ledcWriteTone(BUZZER_CHANNEL, freq);
  ledcWrite(BUZZER_CHANNEL, 128); // 50% duty cycle = loud sound
}
...
void loop() {
  ...
  while (Serial.available() > 0) {
    ...
    if (c == '\r' || c == '\n') {
      ...
      // Flexible matching for DETER:COW, DETER: COW, etc.
  ...
  if (active) {
    if (millis() - startTime > 5000) {
      ...
      // Flash strobe LED and alternate tone frequency every 150ms
      if (millis() - lastFlash >= 150) {
        ...
        if (flashState) {
          playSiren(2400); // 2400Hz siren tone with 50% duty cycle
        } else {
          playSiren(1500); // 1500Hz siren tone with 50% duty cycle
        }
      }
    }
  }
