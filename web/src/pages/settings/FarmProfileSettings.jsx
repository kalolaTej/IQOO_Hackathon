...
export const FarmProfileSettings = () => {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        <h1 className="text-xl sm:text-2xl font-black text-[#0f172a]">{t('settings.title', 'Farm Profile & Platform Settings')}</h1>
      ...
      {saved && (
        <div className="p-3.5 bg-[#dcfce7] border border-[#bbf7d0] text-[#15803d] rounded-xl text-xs font-bold shadow-2xs">
          {t('settings.savedSuccess', '✓ Profile and settings saved successfully!')}
        </div>
      )}

      <form onSubmit={handleSave} className="bg-white rounded-2xl p-4 sm:p-6 border border-slate-200 shadow-xs space-y-5 sm:space-y-6">
        {/* Language Selection */}
        <div className="p-3.5 sm:p-4 bg-[#f0fdf4] rounded-xl border border-[#bbf7d0] space-y-2">
        ...
        <div className="space-y-2.5">
          ...
          <label className="flex items-start gap-2.5 text-xs text-slate-700 cursor-pointer">
            <input type="checkbox" defaultChecked className="accent-[#047857] w-4 h-4 mt-0.5 shrink-0" /> 
            <span>{t('settings.smsMandi', 'SMS alerts for ANPR Mandi queue token updates')}</span>
          </label>
          <label className="flex items-start gap-2.5 text-xs text-slate-700 cursor-pointer">
            <input type="checkbox" defaultChecked className="accent-[#047857] w-4 h-4 mt-0.5 shrink-0" /> 
            <span>{t('settings.smsSiren', 'Real-time sound siren push for perimeter animal intrusions')}</span>
          </label>
          <label className="flex items-start gap-2.5 text-xs text-slate-700 cursor-pointer">
            <input type="checkbox" defaultChecked className="accent-[#047857] w-4 h-4 mt-0.5 shrink-0" /> 
            <span>{t('settings.smsPrice', 'Agmarknet daily price advisory notifications')}</span>
          </label>
        </div>

        <div className="pt-2">
          <button type="submit" className="w-full sm:w-auto px-6 py-3 sm:py-2.5 bg-[#047857] text-white rounded-xl text-xs font-bold hover:bg-[#065f46] shadow-xs transition-colors cursor-pointer text-center">
