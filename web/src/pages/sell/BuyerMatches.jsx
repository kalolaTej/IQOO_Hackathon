  ...
  return (
    <div className="space-y-4 sm:space-y-6 max-w-4xl mx-auto">
      ...
      <div>
        ...
        <h1 className="text-xl sm:text-2xl font-black text-[#0f172a] mt-1">Verified Buyer Offers & Direct Contracts</h1>
      ...
      <div className="space-y-3 sm:space-y-4">
        {buyers.map((b) => (
          <div
            key={b.id}
            className="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200 shadow-xs hover:shadow-md transition-shadow flex flex-col md:flex-row md:items-center justify-between gap-4 sm:gap-5"
          >
            <div className="space-y-1.5 flex-1">
              <div className="flex flex-wrap items-center gap-2">
                <span className="font-extrabold text-sm sm:text-base text-[#0f172a]">{b.name}</span>
              ...
              <div className="text-xs text-slate-600 flex flex-wrap items-center gap-2 sm:gap-3 pt-0.5">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between md:flex-col md:items-end gap-3 shrink-0 pt-3 md:pt-0 border-t md:border-t-0 border-slate-100">
              <div className="text-left md:text-right">
                <div className="text-lg sm:text-xl font-black text-[#0f172a] font-data-tabular">₹{b.price} / Qtl</div>
                <div className="text-[11px] text-[#047857] font-bold">Requirement: {b.qty}</div>
              </div>

              <button
                onClick={() => handleOpenContractModal(b)}
                className="w-full sm:w-auto px-4 py-2.5 bg-[#047857] hover:bg-[#065f46] text-white rounded-xl text-xs font-black shadow-xs transition-colors cursor-pointer active:scale-98 min-h-[40px] flex items-center justify-center"
              >
          ...
          <div className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-6 max-w-lg w-full max-h-[90vh] overflow-y-auto border border-slate-200 shadow-2xl space-y-4 my-auto">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div>
                ...
                <h2 className="text-base sm:text-lg font-black text-[#0f172a]">{selectedBuyer.name}</h2>
              </div>
              <button
            ...
            <form onSubmit={handleExecuteContract} className="space-y-3.5 text-xs">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 bg-[#f8fafc] border border-slate-200 rounded-xl text-xs font-mono font-bold text-[#0f172a] outline-none focus:border-[#047857]"
                />
              ...
              <div className="p-3 bg-amber-50 rounded-xl border border-amber-200 text-[11px] text-amber-800 space-y-1">
                <div className="font-bold flex items-center gap-1">
                  <ShieldCheck size={13} className="shrink-0" /> SIMULATED ESCROW SETTLEMENT NODE
                </div>
              ...
              <div className="flex items-start gap-2 pt-1">
                <input
                  ...
                  className="w-4 h-4 mt-0.5 rounded text-[#047857] cursor-pointer shrink-0"
                />
              ...
              <div className="pt-2 flex flex-col sm:flex-row sm:justify-end gap-2">
                <button
                  type="button"
                  onClick={() => setShowModal(false)}
                  className="w-full sm:w-auto px-4 py-2.5 bg-slate-100 text-slate-700 font-bold rounded-xl hover:bg-slate-200 cursor-pointer text-center min-h-[40px]"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="w-full sm:w-auto px-5 py-2.5 bg-[#047857] hover:bg-[#065f46] text-white font-extrabold rounded-xl shadow-md cursor-pointer active:scale-98 text-center min-h-[40px]"
                >
