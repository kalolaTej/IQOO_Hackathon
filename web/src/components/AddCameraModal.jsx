...
export default function AddCameraModal({ isOpen, onClose, onCameraAdded, onCameraUpdated, cameraToEdit = null, farms = [] }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-3 sm:p-4 overflow-y-auto animate-fade-in">
      <div className="bg-white rounded-3xl max-w-lg w-[calc(100%-1rem)] sm:w-full shadow-2xl overflow-hidden border border-[#E5E7EB] my-6 sm:my-8 flex flex-col max-h-[90vh]">
        {/* Modal Header */}
        <div className="flex items-center justify-between px-4 sm:px-6 py-3.5 sm:py-4 border-b border-[#E5E7EB] bg-[#FAFBF8] shrink-0">
          <div className="flex items-center gap-2 text-[#2F2F2F] min-w-0">
            <div className="p-2 rounded-xl bg-[#047857]/10 text-[#047857] shrink-0">
              <Camera size={18} />
            </div>
            <div className="min-w-0">
              <h2 className="font-black text-sm sm:text-base text-[#0f172a] tracking-tight truncate">
                {isEditing ? 'Edit IP Camera' : 'Add IP Camera'}
              </h2>
              <p className="text-[11px] text-[#666666] truncate">RTSP / HTTP stream, IP, and field setup</p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg text-[#666666] hover:bg-slate-200/60 transition-colors cursor-pointer shrink-0"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-3.5 sm:space-y-4 overflow-y-auto flex-1 text-xs">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                required
                className="w-full px-3 py-2.5 rounded-xl border border-[#D1D5DB] text-xs font-semibold focus:outline-hidden focus:border-[#047857]"
              />
            </div>
            <div>
              <label className="block font-bold text-[#2F2F2F] mb-1">
              ...
              <select
                ...
                className="w-full px-3 py-2.5 rounded-xl border border-[#D1D5DB] text-xs font-semibold bg-white focus:outline-hidden focus:border-[#047857]"
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="sm:col-span-2">
              ...
              <input
                ...
                className="w-full px-3 py-2.5 rounded-xl border border-[#D1D5DB] text-xs font-mono focus:outline-hidden focus:border-[#047857]"
              />
            </div>
            <div>
              <label className="block font-bold text-[#2F2F2F] mb-1">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {['RTSP', 'HTTP_MJPEG', 'HTTP_SNAPSHOT', 'SIMULATED'].map((proto) => (
                <button
                  ...
                  className={`py-2 px-2 rounded-xl font-extrabold text-[10px] border transition-all cursor-pointer text-center ${
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 bg-slate-50 p-3 sm:p-3.5 rounded-2xl border border-slate-200">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 rounded-xl border border-slate-300 text-xs bg-white focus:outline-hidden focus:border-[#047857]"
              />
            </div>
            <div>
              <label className="block font-bold text-slate-600 mb-1 text-[11px]">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full px-3 py-2 rounded-xl border border-slate-300 text-xs bg-white focus:outline-hidden focus:border-[#047857] pr-8"
                />
                <button
                  type="button"
          ...
          <div className="pt-1">
            <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
              ...
              <button
                ...
                disabled={testing}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-900 text-white text-[11px] font-extrabold cursor-pointer transition-all active:scale-95 disabled:opacity-60"
              >
        ...
        {/* Modal Footer */}
        <div className="px-4 sm:px-6 py-3.5 sm:py-4 border-t border-[#E5E7EB] bg-[#FAFBF8] flex flex-wrap items-center justify-end gap-2.5 sm:gap-3 shrink-0">
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2.5 rounded-xl border border-[#D1D5DB] bg-white text-slate-700 text-xs font-bold hover:bg-slate-100 transition-colors cursor-pointer"
          >
            Cancel
          </button>
          <button
            ...
            disabled={loading}
            className="px-5 py-2.5 rounded-xl bg-[#047857] hover:bg-[#065f46] text-white text-xs font-black shadow-md transition-all active:scale-95 cursor-pointer disabled:opacity-60 flex items-center gap-1.5"
          >
            {loading && <RefreshCw size={14} className="animate-spin" />}
            <span>{isEditing ? 'Save Settings' : 'Save & Monitor'}</span>
          </button>
        </div>
      </div>
    </div>
